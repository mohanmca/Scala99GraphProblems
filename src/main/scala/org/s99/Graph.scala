...
package graph {
  ...
  object Graph extends GraphObjBase {
    ...
    def fromString(string : String) : Graph[String, Unit] = {
      ...
      val nodes = strGraph.foldLeft(Nil : List[String])((list, elem) => if (list.contains(elem)) list else elem :: list).reverse
      val edges = strGraph.filter { edge => edge.contains("-") }.map { edge => edge.split("-") }.map { nodes => (nodes(0), nodes(1), ()) }.toList
